...
class ThemePredictionTask(Task):
    ...
    def evaluate_predictions(
        ...
        if self.config.compute_similarity:
            above_threshold = (
                similarities_tensor >= self.config.similarity_threshold
            ).float()
        ...
        if metadata and all("theme_class" in m for m in metadata if m):
            ...
            try:
                ...
                for i, (pred, ref, meta) in enumerate(
                    zip(predictions, references, metadata)
                ):
                ...
                if pred_classes and ref_classes:
                    ...
                    metrics["num_classified"] = len(
                        [p for p in pred_classes if p != -1]
                    )
...
def create_theme_prediction_task(
    task_name: str = "paragraph_theme_prediction", **kwargs
) -> ThemePredictionTask:
...
def test_theme_prediction_task():
    ...
    for i in range(num_samples):
        ...
        metadata.append(
            {"paragraph_id": f"para_{i}", "theme_class": i % 4, "source": "test_data"}
        )
