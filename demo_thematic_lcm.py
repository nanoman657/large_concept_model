...
class ThematicLCMExample:
    ...
    def forward(self, paragraph_embeddings: torch.Tensor, padding_mask=None) -> torch.Tensor:
        """
        ...
        if padding_mask is not None:
            ...
            theme_embeddings = (x * valid_mask.unsqueeze(-1)).sum(dim=1) / valid_mask.sum(dim=1, keepdim=True)
        else:
    ...
    def predict_themes(self, paragraphs: List[torch.Tensor]) -> torch.Tensor:
        ...
        with torch.no_grad():
            self.model.eval()
            themes = self.forward(batch_tensor, mask_tensor if mask_tensor.any() else None)
            
        return themes


def create_synthetic_paragraphs(num_paragraphs=10, num_themes=4) -> Tuple[List[torch.Tensor], List[int]]:
...
def evaluate_theme_similarity(predicted_themes: torch.Tensor, true_theme_labels: List[int]) -> Dict[str, float]:
        ...
        for i in range(len(theme_predictions)):
            for j in range(i + 1, len(theme_predictions)):
                sim = torch.cosine_similarity(
                    theme_predictions[i].unsqueeze(0),
                    theme_predictions[j].unsqueeze(0)
                ).item()
    ...
    return {
        "within_theme_similarity_mean": np.mean(within_similarities) if within_similarities else 0.0,
        "within_theme_similarity_std": np.std(within_similarities) if within_similarities else 0.0,
        "between_theme_similarity_mean": np.mean(between_similarities) if between_similarities else 0.0,
        "between_theme_similarity_std": np.std(between_similarities) if between_similarities else 0.0,
        "theme_separation": (np.mean(within_similarities) - np.mean(between_similarities)) if within_similarities and between_similarities else 0.0,
    }
...
def demonstrate_thematic_lcm():
    ...
    for name, model in models.items():
        ...
        print(f"  ✓ {name} Model: {param_count:,} parameters, {model.theme_dim}D themes")
    
    # 2. Create synthetic paragraph data
    print("\n2. Creating Synthetic Paragraph Data...")
    paragraphs, theme_labels = create_synthetic_paragraphs(num_paragraphs=20, num_themes=4)
    
    print(f"  ✓ Created {len(paragraphs)} paragraphs with {len(set(theme_labels))} themes")
    ...
    for model_name, model in models.items():
        ...
        print(f"    ✓ Predicted {predicted_themes.shape[0]} themes of dimension {predicted_themes.shape[1]}")
        
        # Evaluate
        metrics = evaluate_theme_similarity(predicted_themes, theme_labels)
        results[model_name] = metrics
        
        print(f"    - Within-theme similarity: {metrics['within_theme_similarity_mean']:.3f} ± {metrics['within_theme_similarity_std']:.3f}")
        print(f"    - Between-theme similarity: {metrics['between_theme_similarity_mean']:.3f} ± {metrics['between_theme_similarity_std']:.3f}")
    ...
    best_model = [name for name, metrics in results.items() if metrics == best_separation][0]
    
    for model_name, metrics in results.items():
        marker = "🏆" if model_name == best_model else "  "
        print(f"{marker} {model_name:6}: Separation = {metrics['theme_separation']:6.3f}")
    ...
    for i in range(len(batch_themes)):
        for j in range(i + 1, len(batch_themes)):
            sim = torch.cosine_similarity(
                batch_themes[i].unsqueeze(0),
                batch_themes[j].unsqueeze(0)
            ).item()
    ...
    var_themes = best_model_obj.predict_themes([short_paragraph, long_paragraph])
    print(f"  ✓ Variable lengths: {short_paragraph.shape[0]} & {long_paragraph.shape[0]} sentences → themes computed")
    
    print("\n🎉 Thematic LCM demonstration complete!")
    print(f"Best performing model: {best_model} (separation: {best_separation['theme_separation']:.3f})")
...
def demonstrate_real_world_application():
    ...
    for i, type_i in enumerate(paragraph_types):
        ...
        for j, type_j in enumerate(paragraph_types):
            sim = torch.cosine_similarity(
                type_embeddings[i].unsqueeze(0),
                type_embeddings[j].unsqueeze(0)
            ).item()
            row += f" {sim:6.3f}"
        print(row)
    
    # Show discriminative power
    max_within = max(torch.cosine_similarity(
        type_embeddings[i].unsqueeze(0), 
        type_embeddings[i].unsqueeze(0)
    ).item() for i in range(len(type_embeddings)))
    
    max_between = max(torch.cosine_similarity(
        type_embeddings[i].unsqueeze(0), 
        type_embeddings[j].unsqueeze(0)
    ).item() for i in range(len(type_embeddings)) for j in range(len(type_embeddings)) if i != j)
...
if __name__ == "__main__":
    ...
    print(f"  - Demonstrated paragraph-level theme prediction")
    print(f"  - Showed model comparison and evaluation metrics")
    print(f"  - Simulated real-world application scenarios")
    print(f"  - Architecture successfully captures thematic information! 🎨")