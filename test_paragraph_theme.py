...
class ThematicTestModel(nn.Module):
    ...
    def forward(
        ...
        if padding_mask is not None:
            ...
            paragraph_repr = (encoded * valid_mask.unsqueeze(-1)).sum(
                dim=1
...
def simulate_sentence_embeddings(
    text: str, target_sentences: int = None
) -> torch.Tensor:
    """
    ...
    if any(
        word in text.lower()
        for word in ["research", "study", "analysis", "experiment", "scientific"]
    ):
        embeddings[0, :, :256] += 0.1  # Scientific theme signal
    elif any(
        word in text.lower()
        for word in ["news", "government", "election", "policy", "politics"]
    ):
        embeddings[0, :, 256:512] += 0.1  # News/Politics theme signal
    elif any(
        word in text.lower() for word in ["game", "sport", "team", "player", "match"]
    ):
        embeddings[0, :, 512:768] += 0.1  # Sports theme signal
    elif any(
        word in text.lower() for word in ["feel", "emotion", "personal", "my", "i was"]
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Test paragraph theme prediction with Thematic LCM"
    )
    ...
    try:
        ...
        if args.json:
            ...
            print("=" * 50)
            print(
                f"📝 Input Paragraph: {results['paragraph'][:100]}{'...' if len(results['paragraph']) > 100 else ''}"
            )
            print(
                f"🤖 Model: {results['model_size']} ({results['model_parameters']:,} parameters)"
            )
