...
def create_synthetic_sentence_embeddings(
    paragraphs: List[str], embed_dim: int = 1024
) -> torch.Tensor:
...
def test_mdd_classifier():
    ...
    try:
        ...
        for criterion_idx, (criterion_name, paragraphs) in enumerate(
            MDD_EXAMPLE_PARAGRAPHS.items()
        ):
            ...
            with torch.no_grad():
                ...
                if (
                    hasattr(model, "theme_classifier")
                    and model.theme_classifier is not None
                ):
                    ...
                    for i, paragraph in enumerate(paragraphs):
                        ...
                        print(
                            f"    Predicted: {pred_criterion} (confidence: {confidence:.3f})"
                        )
                        print(f"    Expected: {criterion_name}")
                        print(
                            "    ✅ Correct"
                            if pred_criterion == criterion_name
                            else "    ❌ Incorrect"
                        )
                        print()
                else:
                    print(f"📈 Theme embeddings shape: {theme_embeddings.shape}")
                    print(
                        "ℹ️  Model outputs continuous embeddings (no classification head)"
                    )
                    ...
                    print(
                        f"  Mean norm: {embeddings_norm.mean():.4f} ± {embeddings_norm.std():.4f}"
                    )
                    print(
                        f"  Mean values: {embeddings_mean.mean():.4f} ± {embeddings_mean.std():.4f}"
                    )
                    ...
                    print(
                        f"  Within-criterion similarity: {off_diag_sims.mean():.4f} ± {off_diag_sims.std():.4f}"
                    )
                    print()
        ...
        for i in range(len(all_embeddings)):
            for j in range(i + 1, len(all_embeddings)):
                sim = torch.cosine_similarity(
                    all_embeddings[i], all_embeddings[j], dim=0
                ).item()
        ...
        print(
            f"  📈 {'Excellent' if discrimination_score > 0.3 else 'Good' if discrimination_score > 0.1 else 'Moderate'} discrimination"
        )
        print()
        ...
        print("📋 Model Summary")
        print("-" * 40)
        print("Architecture: thematic_lcm_mdd_classifier")
