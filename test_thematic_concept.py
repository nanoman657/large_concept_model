...
def test_simple_thematic_model():
    ...
    assert theme_embeddings.shape == (batch_size, 256), (
        f"Expected (3, 256), got {theme_embeddings.shape}"
    )
    assert torch.isfinite(theme_embeddings).all(), (
        "Theme embeddings contain non-finite values"
    )
    ...
    assert not torch.allclose(theme_embeddings, theme_embeddings_masked), (
        "Padding mask had no effect"
    )
